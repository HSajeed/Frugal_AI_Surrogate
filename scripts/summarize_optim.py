...
# 🔎 Find optimizer sweep folders
sweep_folders = [f for f in os.listdir(results_dir) if f.startswith("sweep_optim_")]
print(f"📁 Found optimizer sweep folders: {sweep_folders}")
...
for m in all_metrics:
    print(f"{m['optimizer']:>10} | R² Nu: {m['r2_nu']:.4f}, RMSE Nu: {m['rmse_nu']:.4f} | R² DP: {m['r2_dp']:.4f}, RMSE DP: {m['rmse_dp']:.4f}")
...
# 📈 Plot
opts = [m["optimizer"] for m in all_metrics]
...
plot_path = os.path.join(results_dir, "optimizer_summary_plot.png")
