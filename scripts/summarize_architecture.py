...
# 🔎 Find architecture sweep folders (exclude activation)
sweep_folders = [f for f in os.listdir(results_dir) if f.startswith("sweep_") and not f.startswith("sweep_activation_")]
print(f"📁 Found architecture sweep folders: {sweep_folders}")
...
for m in all_metrics:
    print(f"{m['architecture']:>12} | R² Nu: {m['r2_nu']:.4f}, RMSE Nu: {m['rmse_nu']:.4f} | R² DP: {m['r2_dp']:.4f}, RMSE DP: {m['rmse_dp']:.4f}")

# 💾 Save summary
summary_path = os.path.join(results_dir, "architecture_summary.json")
with open(summary_path, "w") as f:
    json.dump(all_metrics, f, indent=2)
print(f"\n✅ Summary saved to: {summary_path}")

# 📈 Plot
archs = [m["architecture"] for m in all_metrics]
...
ax.set_xticks(x)
ax.set_xticklabels(archs, rotation=45)
ax.set_title("Architecture Performance")
ax.legend()
plt.tight_layout()

plot_path = os.path.join(results_dir, "architecture_summary_plot.png")
